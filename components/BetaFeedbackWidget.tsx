    ...
    <>
      ...
      {!open && (
        <button
          ...
          className="fixed bottom-5 right-5 z-50 bg-rust hover:bg-copper text-white rounded-full w-14 h-14 flex items-center justify-center shadow-lg transition-colors"
      ...
      {/* Feedback panel */}
      {open && (
        <div className="fixed bottom-5 right-5 z-50 w-80 bg-white/5 border border-white/10 rounded-xl shadow-2xl overflow-hidden">
          {/* Header */}
          <div className="bg-rust text-white px-4 py-2.5 flex items-center justify-between">
                  ...
                  {FEEDBACK_TYPES.map(({ value, label }) => (
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-full text-xs font-medium transition-colors ${
                        feedbackType === value
                          ? 'bg-rust text-white'
                          : 'bg-white/8 text-white hover:bg-white/10'
                      }`}
                    >
                      {label}
                    </button>
