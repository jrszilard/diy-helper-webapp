      ...
      {[1, 2, 3].map(group => (
        <div key={group}>
          ...
          <div className="space-y-2">
            {[1, 2].map(item => (
              <div key={item} className="bg-white/5 border border-white/[0.08] rounded-lg p-3 flex items-center justify-between">
...
export function ShoppingSearchSkeleton() {
  return (
    <div className="space-y-3 p-4">
      {[1, 2, 3].map(i => (
        <div key={i} className="p-3 rounded-lg border border-white/[0.08] bg-white/5 space-y-3">
