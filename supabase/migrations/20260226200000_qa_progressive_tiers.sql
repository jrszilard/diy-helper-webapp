...
alter table qa_questions add column if not exists current_tier int not null default 1;
alter table qa_questions add column if not exists tier_payments jsonb not null default '[]';
-- tier_payments format: [{"tier":1,"amount_cents":1500,"payment_intent_id":"pi_xxx","charged_at":"2026-..."},...]

-- ── Expert insight notes ─────────────────────────────────────────────────

alter table qa_questions add column if not exists expert_notes jsonb default null;
...
create index if not exists idx_qa_questions_tier on qa_questions(current_tier) where current_tier > 1;

-- ── RLS for report_corrections (already has policies, add update for experts) ─

drop policy if exists "Experts update own corrections" on report_corrections;
